...
export default class ConnectionRepository {
    ...
    public  insert(id: number, socket: Socket, position: Position, timestamp: number) {
        if (!id) return
        this.repo.recall(id).then(v => {
            if (v?.timestamp! > timestamp)
                return
            this.active.add(id)            
            this.repo.store(id, { socket: socket.id, position, lastPinged: Date.now(), timestamp })
        })
    }
